...
(() => {
  ...
  function hasApplicableAISuggestions(reviewModel = state.receiptReviewModel) {
    ...
    return buildReceiptFormApplyPlan(receipt, {
      availableFields: getAvailableReceiptApplyFields(),
    }).fields.length > 0;
  }
  ...
  function onApplyAISuggestions() {
    ...
    const applied = [];
    const applyPlan = buildReceiptFormApplyPlan(receipt, {
      availableFields: getAvailableReceiptApplyFields(),
    });
    const skipped = applyPlan.skipped.slice();

    for (const field of applyPlan.fields) {
      if (applyReceiptSuggestionField(field)) {
  ...
  function getAvailableReceiptApplyFields() {
    return {
      location: Boolean(elements.locationInput),
      license: Boolean(elements.licenseInput),
      date: Boolean(elements.dateInput),
      time: Boolean(elements.timeInput),
      amount: Boolean(elements.amountInput),
    };
  }

  function applyReceiptSuggestionField(field) {
    const inputByField = {
      location: elements.locationInput,
      license: elements.licenseInput,
      date: elements.dateInput,
      time: elements.timeInput,
      amount: elements.amountInput,
    };
    const input = inputByField[field.field];

    if (!input || !hasReviewValue(field.value)) {
      return false;
    }
